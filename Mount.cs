...
namespace BasicApi
{
    ...
    public abstract class Mount
    {
        ...
        public virtual String TalkWithAxis(AXISID Axis, char Command, string cmdDataStr)
        {
            ...
            lock (mConnection)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    try
                    {
                        ...
                        // send the request
                        //Console.Write(Axis.ToString());
                        if (Command != 'f' && Command != 'j') { Console.WriteLine(Command.ToString() + " " + cmdDataStr); }

                        //Console.Write(cmdDataStr);
                        //Console.WriteLine();
        ...
        public long RadSpeedToInt(AXISID Axis, float RateInRad)  //mcs made public
        {
